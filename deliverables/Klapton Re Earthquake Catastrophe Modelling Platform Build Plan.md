...
> *Changed after 1.8.* CASS is a research tool. Its results are not a basis for pricing, reserving, capital or underwriting decisions. Where this plan describes a governed decision platform, a production release, or controls that only such a service needs, that description is overtaken ([ADR 15](../platform/docs/adr/0015-research-tool-and-gem-permission.md)). What was built to that description stays as built, and the remaining work is scoped for research. GEM Foundation has since given explicit written permission to use the data and models it makes publicly available, PuSGeN 2024 included, for the use KRE described.
...
> *Changed after 1.8.* Nepal acquires no hazard. Indonesia and Nepal were chosen as test countries, Indonesia is covered, and Nepal keeps its grid and its keys without a source model.
...
> *Changed after 1.8.* GEM Foundation has replied to KRE's email of 11 September 2026. It acknowledged that the use KRE described does not violate its terms of use, and granted explicit permission under the Data and Models section of its terms, which covers everything GEM makes publicly available: the Global Exposure and Vulnerability models, and the national hazard models in its mosaic, such as PuSGeN 2024 ([ADR 15](../platform/docs/adr/0015-research-tool-and-gem-permission.md)). The authors and GEM must be credited, and anything redistributed carries the same licence. Legal review is still needed before model data or a derived package leaves KRE.
...
| Event representation | Occurrence per event, recorded on each package by its converter-candidate approval; the sign-off is dropped and the study is CASS's own | Open: CASS to measure and decide, tracker item 26 | Controls frequency, uncertainty, correlation and footprint probabilities |
...
| Nepal hazard source | No source model acquired, and none will be: Indonesia and Nepal were test countries. A finished CASS builds the grid and the keys for whatever country its user works on, which is tracker items 24 and 25 | Closed after 1.8 | Pilot data is disposable; the native build path is what matters |
| Purpose | CASS is a research tool; its results are not a basis for pricing or reserving ([ADR 15](../platform/docs/adr/0015-research-tool-and-gem-permission.md)) | Changed after 1.8 | Decides which governance and production controls are worth building |
| Model data rights | Everything GEM makes publicly available, PuSGeN 2024 included, used under GEM Foundation's written permission; data GEM does not publish under one internal-use basis ([ADR 7](../platform/docs/adr/0007-internal-use-licence-basis.md), [ADR 15](../platform/docs/adr/0015-research-tool-and-gem-permission.md)) | Changed after 1.8 | Controls attribution, and whether data or a derived package may leave KRE |
...
| Exposure enrichment | Reported data first; GEM-derived conditional priors only for missing fields; compare approved assumption scenarios. Applied within a run as a vulnerability set the engine selects ([ADR 14](../platform/docs/adr/0014-assumption-sets-as-vulnerability-sets.md)); the More Robust and More Vulnerable tilts are draft sensitivities awaiting the exposure-enrichment gate | Proposed, mechanism implemented | Prevents false precision while allowing incomplete facultative portfolios to be modelled |
...
| Vulnerability source | GEM v2026.0.0 functions, discretised with a mean-preserving method and a reconstruction check on every bin; used under GEM Foundation's written permission ([ADR 15](../platform/docs/adr/0015-research-tool-and-gem-permission.md)) | Partially confirmed | Controls model credibility, cost and publication rights |
...
| M5 Loss | Met: ground-up, insured and reinsurance run with keys reconciliation, an assumption set applied within a run and compared live against the baseline, currency conversion under an approved rate, and the financial structure read, reconciled and built on the platform | — |
| M6 Product | Met: result approval, export, comparison, EP chart, event loss table, loss by area-peril cell with its map, scenario ranges across assumption sets, and the financial structure workspace | — |
...
| Assumption sets | Enrichment applied to the exposure | Re-weighted GEM mixtures carried as vulnerability sets the engine selects per analysis; no value moves; the pilot tilts are draft | [ADR 14](../platform/docs/adr/0014-assumption-sets-as-vulnerability-sets.md) |

> *Changed after 1.8.* Two further changes. CASS is a research tool rather than a governed decision platform, and the data and models GEM makes publicly available are used under GEM Foundation's written permission ([ADR 15](../platform/docs/adr/0015-research-tool-and-gem-permission.md)).

### What remains

Buildable in code, scoped for research after 1.8: pointing CASS at the GEM release on the user's own device; and backup and restore of research work, postponed on 14 September 2026 as not yet necessary. CASS is also to research, test and decide the multi-IMT representation, the realisation-weighting rule, the footprint storage format and the event representation. Not pursued after 1.8: separately deployed keys and converter services, MFA and single sign-on, SBOMs, and a signed release bundle.

Delivered since 1.8, by 14 September 2026: a grid, a vulnerability set and a model version built on the platform for any country, with runs and the OpenQuake reference comparison following the enrichment and GEM location each set records; the Nepal prototype removed as test data; the Cohort B geocoding sensitivity; loss by area-peril cell with its map; scenario ranges across assumption sets; a financial structure built on the platform; CI coverage of the patched worker and an integration workflow; and every pulled image pinned by digest so a run can be repeated.
...
Settled after 1.8: the sign-off of the event representation study is dropped, though the study itself stays as CASS's own work; legal review before model data leaves KRE is cleared; and Nepal acquires no hazard, because a finished CASS builds the grid and keys for whatever country its user works on. What CASS is to research and decide is the multi-IMT representation, the realisation-weighting rule, the footprint storage format and the event representation, as tracker items 21, 22, 23 and 26. Building a grid and a vulnerability set for any country natively is tracker items 24 and 25.
