...
- The Indonesian hazard was rebuilt on the live stack under ADR 18 and the same
  books re-run on it. The new set covers the same thousand simulated years as one
  fifty-year event set along each of twenty sampled paths rather than twenty sets
  along one path: 27,001 events over the same 962 cells, 13,588,639 footprint
  rows, nothing clipped, frequency preserved over 1,000 periods. With the grid,
  the vulnerability set and the books held fixed, ground-up average annual loss
  moved between −0.3% and +6.8% while the 1,000-year loss rose between 51% and
  93%. The two footprints say why: the body is the same distribution — mean peak
  PGA bin 11.57 against 11.60, 22.15% against 22.39% of events peaking at bin 18
  or above — while the strongest shaking anywhere rises from bin 42 to bin 50.
  Those far return periods are the worst one or two years of a thousand-year
  catalogue and are not precisely estimated by either run. Every Indonesian
  result recorded before this rebuild sits on the single-path hazard, and it is
  their tail rather than their average that the rebuild moves.
...
| M3 Hazard | Partly met | OpenQuake adapter and hazard runs; PuSGeN 2024 on the Jakarta–Bandung region; published Vs30 joined to 37% of cells; benchmark comparison machinery; the realisation-weighting rule decided, built, and the Indonesian hazard rebuilt under it ([ADR 18](adr/0018-sampled-paths-pooled-as-one-catalogue.md)) | Approved benchmark curves; a full-country run. Nepal acquires no hazard: it was a test country and Indonesia is covered |
...
| 25 | Build a vulnerability set for any country GEM covers | §6, §8 | Done: the catalogue is read from the release on the installation, and the enrichment — the design eras, each with its reason — is posted with it rather than compiled in. The set's version follows the enrichment, because the enrichment is the assumption behind every function. An era table that is out of order, ends before today, or names a design level GEM does not use is refused. "Any country" had not been measured, and was not true: 84 of the 215 countries GEM v2026.0.0 publishes could not be built. The reader took the occupancy to be the fifth segment of a taxonomy string, and GEM states a roof or an irregularity before it for 75 countries — Turkey and Greece among them; two countries are named differently in the two repositories (Turkey is published as Turkiye, Cape Verde as Cabo Verde) so their functions never met their stock; and China's summary was refused by a rule that inferred double counting from the file's shape, which its national totals disprove. 208 now build, checked country by country against the release. The seven that do not are the United States, Canada and their territories, which GEM publishes in HAZUS classes rather than in its own taxonomy; they are listed with that reason and cannot be chosen |
...
| 36 | Stream the footprint to its files instead of holding it in memory | §7, §15 | Done: found while sizing the national Indonesian run. The datastore reader streams under a row budget — item 7 measured its peak at 35 MB — and every caller undid that immediately, collecting the rows it emitted into one list and copying that into a tuple. A footprint row is a frozen slotted dataclass of 80 bytes including its pointer, so the 13.6M-row regional conversion held 1.1 GB of interpreter objects and the national one, 55 times the cells, would have needed 32–60 GB against the 31.2 GB the containers see. It would have failed after the OpenQuake calculation had already run, losing hours. Rows now go straight to one CSV per measure, and the conversion's whole report — rows per measure, the cells and events touched, probabilities summing to one, a bin appearing twice, the worst group — is counted as they pass: the stream is already in the order an Oasis footprint wants, every check is per-group and a group is contiguous in it, and every aggregate is bounded by the event set or the grid rather than by the row count. The files upload from disk rather than through a `bytes`. Verified on the real conversion: rebuilding the 13.6M-row Jakarta–Bandung footprint from the engine's own datastore reproduced all four registered tables byte for byte, at a peak of 0.26 GB |
| 37 | Bin the ground motion in arrays rather than one value at a time | §7, §15 | Done: the conversion was 64% of a hazard run — 212s against the engine's 120s on the regional twenty-path run — because it spent tens of millions of iterations building a `Decimal` per ground-motion value by formatting a float into a string, and rebuilding a 50-element list of bin bounds on each of 765,000 lookups. The bounds are now worked out once, and the datastore hands the accumulator one array per event and measure instead of an object per value: the binning is a `searchsorted`, the counting a `unique`. Adopting float rounding in place of decimal rounding needed proof rather than argument, so the two were compared over all 27,590,900 ground-motion values of the Indonesian calculation and none differed; where a scaled value sits on a rounding tie — about two in every million — the decimal path still decides it, because the two directions can straddle a bin edge. The real 13.6M-row conversion fell from 98s to 56s and its four tables stayed byte for byte identical. Conversion is no longer the dominant cost of a hazard run: it is now about a third of it, and the engine is the rest |
| 38 | A fresh installation can actually be stood up | §17, §18 | Done: found by flushing the volumes and starting over, which is the only thing that runs it. Postgres executes `/docker-entrypoint-initdb.d` once, against an empty data directory, and the script that creates the Oasis and celery databases said `CREATE DATABASE oasis OWNER CURRENT_USER` — a syntax error, because `CREATE DATABASE` takes a literal role name there and, unlike `ALTER`, does not accept the keyword. The whole init script failed, postgres exited 3, and every service depending on it refused to start. No existing deployment had ever seen it: their data directories predate the script, so it never ran. The clause is dropped rather than corrected, because `CREATE DATABASE` already gives the database to the role executing it, which is the control-plane role the comment intends. Verified by deleting all six volumes and bringing the full stack up from nothing: both engine databases created, migrations applied, the demonstration workspace seeded, and CASS, Oasis and OpenQuake all answering |
