...
| --- | --- | --- |
| Event representation | Decided ([ADR 17](docs/adr/0017-an-oasis-event-is-a-simulated-occurrence.md)). Occurrence per event, recorded on each package by its converter approval; rupture binning measured against it on the live event set and not built | Controls frequency, uncertainty, correlation and footprint probabilities |
| Multi-IMT representation | Decided ([ADR 16](docs/adr/0016-multi-measure-classes-as-sub-peril-channels.md)). Correlated area-peril channels for classes resolving to one measure; a class spanning measures reaches the engine as one earthquake sub-peril item per measure, each answered by its channel's function scaled by the channel's share, with the terms the engine receives scoped to every earthquake peril. Measured against an OpenQuake reference calculation on three books | Determines whether GEM vulnerability can be represented faithfully in Oasis |
| Realisation weighting | Decided ([ADR 18](docs/adr/0018-sampled-paths-pooled-as-one-catalogue.md)). Twenty sampled paths pooled as one catalogue of the same thousand simulated years; enumeration refused, because its branches carry unequal weights. One path sat between 0.84 and 1.24 of the model's own weighted mean depending on the draw, and twenty cost the same to run | A single path is one view of the hazard, not the model's answer |
| Oasis static storage format | Decided ([ADR 19](docs/adr/0019-footprint-stays-a-ktools-binary.md)). The ktools binary CASS writes today; compressed binary measured as the reserve (four times smaller, same read speed, about a minute more to build at national scale), Parquet rejected | A national footprint is 9.5 GB, and every format gave the same loss to the cent |
