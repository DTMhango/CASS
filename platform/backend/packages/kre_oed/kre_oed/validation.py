...
#: OED financial term columns KRE does not yet calculate. Section 15 requires
...
def _check_columns(result: ReadResult, out: FindingSet) -> None:
    ...
    for name in result.unrecognised_columns:
        out.add(
            fnd.make(
                "unrecognised_column",
                f"{result.kind.label} column {name} was not interpreted by KRE.",
                file_kind=str(result.kind),
                field=name,
            )
        )
        ...
        for name, spec in specs.items():
            ...
            if spec.allowed is not None and str(value).upper() not in spec.allowed:
                out.add(
                    fnd.make(
                        "invalid_code",
                        f"{spec.business_label} value {value} is not supported by KRE. "
            ...
            if row.text(name) != "":
                out.add(
                    fnd.make(
                        ...
                        f"{name} carries a value KRE does not calculate in this release.",
                        file_kind=str(result.kind),
                        row_number=row.row_number,
                        field=name,
                        value=row.text(name),
