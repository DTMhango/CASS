"""The pinned OED schema subset that KRE supports.
...
validator reports it as an unrecognised column so an analyst can see that KRE
...
class FieldSpec:
    """One OED column as KRE interprets it.
...
LOCATION_FIELDS: tuple[FieldSpec, ...] = (
    ...
    FieldSpec("OEDVersion", DataType.TEXT, "OED version",
              business_help="Recorded for lineage; KRE pins the supported version."),
)
...
ACCOUNT_FIELDS: tuple[FieldSpec, ...] = (
    ...
    _rate("LayerParticipation", "Signed share",
          "The share KRE writes on this layer, as a proportion."),
...
#: Earthquake peril codes KRE recognises. Section 6 requires each country
...
#: Sub-perils KRE can currently model. Everything else in a covered peril
...
def expand_perils(value: str) -> tuple[str, ...]:
    ...
    which of those KRE actually models.
    """
    codes: list[str] = []
...
def unmodelled_subperils(covered: Sequence[str]) -> tuple[str, ...]:
    """Return covered earthquake sub-perils that KRE does not model.
