...
Every stored object is addressed by a ``cass://`` URI and carries a content
...
URI_SCHEME = "cass"
_URI_PATTERN = re.compile(r"^cass://(?P<bucket>[a-z0-9][a-z0-9.\-]{1,62})/(?P<key>.+)$")
...
def parse_uri(uri: str) -> tuple[str, str]:
    """Split a ``cass://`` URI into bucket and key, rejecting anything unsafe."""
    match = _URI_PATTERN.match(uri)
    if not match:
        raise InvalidArtifactURI(f"not a CASS artifact URI: {uri!r}")
    key = match.group("key")
        ...
        with open(source, "rb") as handle:
            self._client.upload_fileobj(
                ...
                ExtraArgs={
                    ...
                    "Metadata": {
                        "cass-checksum": checksum,
                        "cass-retention": str(retention),
                        "cass-access": str(access),
                    },
                },
            )
...
def store_from_env() -> ArtifactStore:
    ...
    ``CASS_ARTIFACT_BACKEND`` selects ``filesystem`` (default) or ``s3``.
    """
    backend = os.environ.get("CASS_ARTIFACT_BACKEND", "filesystem").lower()
    if backend == "filesystem":
        return FilesystemArtifactStore(os.environ.get("CASS_ARTIFACT_ROOT", ".artifacts"))
    if backend == "s3":
        import boto3  # imported lazily so local installs need no AWS SDK

        client = boto3.client(
            "s3",
            endpoint_url=os.environ.get("CASS_S3_ENDPOINT") or None,
            region_name=os.environ.get("CASS_S3_REGION", "us-east-1"),
        )
