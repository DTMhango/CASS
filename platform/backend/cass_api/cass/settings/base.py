"""Base settings for the CASS control plane.
...
SECRET_KEY = env("CASS_SECRET_KEY", "insecure-development-key-replace-in-every-deployment")
DEBUG = env_bool("CASS_DEBUG", False)
ALLOWED_HOSTS = env_list("CASS_ALLOWED_HOSTS", "localhost,127.0.0.1")
...
ASGI_APPLICATION = "cass.asgi.application"
...
DATABASES = {
    "default": {
        ...
        "NAME": env("CASS_DB_NAME", "cass"),
        "USER": env("CASS_DB_USER", "cass"),
        "PASSWORD": env("CASS_DB_PASSWORD", "cass"),
        "HOST": env("CASS_DB_HOST", "localhost"),
        "PORT": env("CASS_DB_PORT", "5432"),
        "CONN_MAX_AGE": int(env("CASS_DB_CONN_MAX_AGE", "60")),
    }
}
...
CASS_ARTIFACT_BACKEND = env("CASS_ARTIFACT_BACKEND", "filesystem")
CASS_ARTIFACT_ROOT = env("CASS_ARTIFACT_ROOT", str(BASE_DIR.parent.parent / ".artifacts"))
CASS_S3_ENDPOINT = env("CASS_S3_ENDPOINT")
CASS_S3_REGION = env("CASS_S3_REGION", "us-east-1")

#: Buckets by purpose, so a retention or access rule can be applied to a whole
#: class of objects rather than object by object.
CASS_BUCKETS = {
    "upload": env("CASS_BUCKET_UPLOAD", "cass-upload"),
    "portfolio": env("CASS_BUCKET_PORTFOLIO", "cass-portfolio"),
    "model": env("CASS_BUCKET_MODEL", "cass-model"),
    "hazard": env("CASS_BUCKET_HAZARD", "cass-hazard"),
    "result": env("CASS_BUCKET_RESULT", "cass-result"),
}
...
CASS_FIXTURE_ROOT = env("CASS_FIXTURE_ROOT", str(BASE_DIR.parent / "tests" / "fixtures"))
...
CELERY_RESULT_BACKEND = env("CASS_RESULT_BACKEND", "redis://localhost:6379/1")
...
SPECTACULAR_SETTINGS = {
    "TITLE": "CASS API",
    "DESCRIPTION": (
        "The control-plane API of CASS, the Catastrophe Analytics and Scenario "
        "Suite. The React application calls only this API; "
...
CORS_ALLOWED_ORIGINS = env_list("CASS_CORS_ORIGINS", "http://localhost:5173")
...
CSRF_TRUSTED_ORIGINS = env_list("CASS_CSRF_TRUSTED_ORIGINS", "http://localhost:5173")
...
LOGGING = {
    ...
    "root": {"handlers": ["console"], "level": env("CASS_LOG_LEVEL", "INFO")},
    "loggers": {
        "django.db.backends": {"level": "WARNING", "handlers": ["console"], "propagate": False},
        "cass": {"level": env("CASS_LOG_LEVEL", "INFO"), "handlers": ["console"], "propagate": False},
    },
}
