...
class PlatformInfoView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        from kre_oed.schema import OED_SCHEMA_VERSION

        return Response(
            {
                "api_version": "1.0.0",
                "oed_schema_version": OED_SCHEMA_VERSION,
                "compatibility_matrix": settings.KRE_COMPATIBILITY_MATRIX,
                "execution_profiles": settings.KRE_EXECUTION_PROFILES,
                "default_execution_profile": settings.KRE_DEFAULT_EXECUTION_PROFILE,
                "artifact_backend": settings.KRE_ARTIFACT_BACKEND,
                "engines": {
                    "openquake": settings.KRE_OPENQUAKE_URL,
                    "oasis": settings.KRE_OASIS_API_URL,
                    "keys": settings.KRE_KEYS_SERVICE_URL,
                    "converter": settings.KRE_CONVERTER_URL,
                },
            }
        )
