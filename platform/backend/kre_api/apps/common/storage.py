...
def get_store() -> ArtifactStore:
    ...
    """
    backend = settings.KRE_ARTIFACT_BACKEND.lower()
    if backend == "filesystem":
        return FilesystemArtifactStore(settings.KRE_ARTIFACT_ROOT)
    if backend == "s3":
        import boto3

        client = boto3.client(
            "s3",
            endpoint_url=settings.KRE_S3_ENDPOINT or None,
            region_name=settings.KRE_S3_REGION,
        )
...
def bucket(purpose: str) -> str:
    ...
    try:
        return settings.KRE_BUCKETS[purpose]
