...
selects the pipeline from ``kre_core.runs``; the kind-specific detail lives in a
...
class Run(BaseModel):
    ...
    def transition(
        ...
        *,
        ...
        The state machine lives in ``kre_core`` so that workers enforce the same
