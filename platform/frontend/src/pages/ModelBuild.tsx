...
import type { Approval, HazardSet, ModelVersion } from "@/api/types";
  ...
  return (
    ...
    >
      <ul className="gate-list">
        {open.map((approval) => (
          <OpenGate key={approval.id} approval={approval} mayDecide={mayDecide} />
        ))}
      </ul>
    </Card>
  );
...
function OpenGate({
  ...
  return (
    <li className="gate">
      ...
      {approval.rationale ? (
        <p className="gate__rationale">{approval.rationale}</p>
      ) : null}
